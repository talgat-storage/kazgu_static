...
function init3()
{
  ...
  imY03 = height3-9-imHeight3/2; imY3 = imY03;
...
//		n_up = nFirstEnergy+1+Math.round((nMaxEnergy-3)*Math.random());
        ...
        if (Utmp<0) Utmp = 0;
        if(ntext<3){if(U>0)setStar(Utmp,imY3);else onStart()};
		if(Math.abs(U-energys[nFirstEnergy+nEnergy-1].energy)<0.08)isExcit=true; else isExcit=false;
		if(ntext<3)paint4();	
	}	
//==========================================================================
function paint3()
...
var l = 0;
var dxp;
  if(current_gaz<3)dxp=10; else dxp=8;
  context3.strokeStyle="yellow";
  context3.beginPath();
