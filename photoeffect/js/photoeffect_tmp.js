...
if(mouse.x>imX+dx&&mouse.x<imX+dx+imWidth&&mouse.y>imY0&&mouse.y<imY0+imHeight){
 bBeginDrag=true; dx = imX - mouse.x+ dx;}
...
if(mouse.x>imX1+dxC&&mouse.x<imX1+dxC+25&&mouse.y>4&&mouse.y<4+15){
 bDragCrawl=true; dxC = imX1 - mouse.x+ dxC;}
...
function opp(a){
	switch (a){
		case 1:dxL=dxL%4;dxL++;if(imX1<79)if(dxL==1)imX1+=1; lambda+=dlambda; findColor();showVA();break;
		case 2:if(imX2<333)imX2+=6;nInt =(imX2-imX20)/6;showVA();break;
		case 3:dxL=dxL%4;dxL++;if(imX1>imX10)if(dxL==1)imX1-=1; lambda-=dlambda; findColor();showVA();break;
		case 4:if(imX2>imX20)imX2-=6;nInt =(imX2-imX20)/6;showVA();break;
	}
	paint();
}
...
for( i=0;i<6;i++)
{
	var r=p(rgb[0],2*i*i); g=p(rgb[1],2*i*i); b=p(rgb[2],2*i*i);
...
function findC( n, l){
	...
	var i=0;
	while((l-400)>r[i+1])i++;
	var k = [[0,0,0,1,2,2],[0,1,2,2,-1,0],[2,2,-1,0,0,0]];
	var Amax = [255,200,255];
	var Amin = 0;
	switch(k[n][i]){
	case 2:return Amax[n];
	case 1:case -1:return (Amax[n]+Amin+k[n][i]*(Amin-Amax[n]))/2+k[n][i]*Math.round((l-400-r[i])*(Amax[n]-Amin)/(r[i+1]-r[i]));
	default: return Amin;				   
	}
}
// ============================================
...
// ============================================  
function showVA(){
		if(OnOff)U = 0.2*(imX0-imX);else U=0;
...
function getAnsw(){
	...
	var answer=0;
  switch(ntext){
	case 4:	answer=getIa(0);break;
	case 5:answer=-Ukr;break;
	case 6:if(Ukr>0)answer=593*Math.sqrt(Ukr);break;
	case 7:answer=1241/AeV[subst];
  }
