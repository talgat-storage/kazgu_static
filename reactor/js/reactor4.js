...
//alert("answer ntask= "+ntask)
	if(ncase[0]>0){
		switch(ntask-3){
		case 1: answ = (ncase[1]+ncase[2])/ncase[0];;break;
		case 2: answ = (ncase[2])/ncase[0]; break;
		case 3: answ = 2; break;		
		case 4: answ = (ncase[3]*100)/(ncase[0]+ncase[1]+ncase[2]+ncase[3]);
	}// switch
	}// if
	else return "";
...
//console.log("move_n nstep= "+nstep+" isMoveN= "+isMoveN+" n= "+n+" count= "+count+" xn= "+xn+" yn= "+yn)
	  ...
	  yn +=dyn;
      n++;
   if(n==count){clearInterval(Btimer); xf=xn;yf=yn;init_process(); dispatcher()}
...
function dispatcher()
  {
	...
	ya = defineNuclear(yn);
	//initXY();
	if(number_process<4){isExcit = true; paint();}
//  console.log("dispatcher isExcit= "+isExcit)
	
	switch(number_process) {
    case 1:setN(0);colorG="rgb(210,240,255)";
console.log("dispatcher xn= "+xn+" yn= "+yn)
    initFoton();
	  break;
    case 2:setN(1);colorG="rgb(135, 206, 250)"; initFoton();
	  break;
    case 3:setN(2);run_nucl(); //  alert("dispatcher");
	  break;
    case 4: isNeutron = true; setN(3);setTimeout(run_n,200);
	  break;
    default:
  }
  }
...
function initFoton()
{
  ...
  FotonFaza(dirF);
	switch(dirF) {
    case 0:dxF = dx0; dyF = -dy0; number=5;
	  break;
    case 1:dxF = -dx0; dyF = -dy0; number=2;
	  break;
    case 2:dxF = -dx0; dyF = dy0; number=3;		
	  break;
    case 3:dxF = dx0; dyF = dy0; number=4;
	  break;
    default:dxF = 0; dyF = 0;
  }
	xf =xn-((dxF>0)?0:30);
