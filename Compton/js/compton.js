...
function paint()
{
  ...
  context = elem.getContext('2d');
	...
	if(isOnOff){	context.drawImage(rentg_plant1,0,269,35,23, width-40, height-30,35,23);return;	}
	else{
...
//alert("getAnswer n="+n+" hnu="+hnu+" teta_ask="+teta_ask);
  switch(n){
	case 3:	answer=hnu/(1+hnu/511*(1-Math.cos(Math.PI*(teta_ask)/180)));break;
	case 4:answer=hnu-hnu/(1+hnu/511*(1-Math.cos(Math.PI*(teta_ask)/180)));break;
	case 5: Tkin=hnu-hnu/(1+hnu/511*(1-Math.cos(Math.PI*(teta_ask)/180)));
			answer=Math.sqrt(1-Math.pow(511/(Tkin+511),2));
			break;
	case 6:var fi=Math.atan(1/(Math.tan(Math.PI*teta_ask/360)*(1+hnu/511))); 
			answer=180*fi/Math.PI;break;
	case 7:answer=1.241/hnu;break;
	case 8:answer=2.426*(1-Math.cos(Math.PI*teta_ask/180));
  }
